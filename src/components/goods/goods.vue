      ...
      <div class="foods-wrapper" ref="foodsWrapper">
        <ul>
          <li class="food-list" ref="foodList">
            ...
            <ul>
              <li v-for="food in this.dishesInfo" class="food-item border-1px">
                <div class="icon">
                ...
                <div class="content">
                  ...
                  <div class="price">
                    <span class="now">￥{{food.discountprice}}</span><span class="old"
                                                                  v-show="food.price">￥{{food.price}}</span>
                  </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      //获取菜品信息
      getDishesInfo(){
        let myurl = publicDom.main_url+'queryDishesByRid';
        this.$http.post(myurl,{"rid":this.restaurantDetail.id},{emulateJSON:true}).then((res)=>{
            let list = res.data;
          if(list.code == 200){
              this.dishesInfo = list.list;
              //等到Dom更新完毕才执行bs插件
              this.$nextTick(() => {
                this._initScroll();
              });
